...
def main():
    st.title("CropEasy (Crop Recommender)...")
    ...
    st.subheader(option)
    sn=st.number_input('NITROGEN (N)', 0.0, 150.0)
    sp=st.number_input('PHOSPHOROUS (P)', 0.0, 150.0)
    pk=st.number_input('POTASSIUM (K)', 0.0, 210.0)
    pt=st.number_input('TEMPERATURE', 0.0, 50.0)
    phu=st.number_input('HUMIDITY', 0.0, 100.0)
    pPh=st.number_input('Ph', 0.0, 14.0)
    pr=st.number_input('RAINFALL', 0.0, 300.0)
